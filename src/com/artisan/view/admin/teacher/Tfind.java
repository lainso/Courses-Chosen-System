 ...
 * @author 1
 */
public class Tfind extends JFrame {
    ...
    private void initComponents() {
        ...
        label2.setBounds(new Rectangle(new Point(405, 225), label2.getPreferredSize()));
    ...
    private void getData(user user){
        ...
        try {
            ...
            ResultSet rs = ad.listt(con,user);
            while (rs.next()){
                v.add(rs.getInt("uid"));
                v.add(rs.getString("uname"));
                v.add(rs.getString("upass"));
            }
            for (int row=0; row<(v.toArray().length)/3; row++){
                for (int col = 0; col<3; col++){
                    dtm.setValueAt(v.get(3*row+col),row,col);
                }
            }
